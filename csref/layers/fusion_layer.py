...
class CollectDiffuseAttention(nn.Module):
    ...
    def forward(self, q, kc,kd, v, mask=None):
        ...
        # attention scores along the image dimension
        attn_col = self.dropout_c(attn_col)
        # attention scores along the channel dimension
        ...
        output=torch.bmm(attn_dif,attn)  # restore to original feature scale
...
class GaranAttention(nn.Module):
    ...
    def forward(self, q, v, mask=None):
        ...
        attn=output  # (b, c, h, w)
        m_attn=self.w_m(attn).view(sz_b, h_v*w_v)  # (b, 1, h, w) --> (b, h*w)
        output=self.layer_norm(output)
        output= output+residual
        output=self.layer_acti(output)  # (b, c, h, w)
...
class SimpleFusion(nn.Module):
    ...
    def forward(self, x, y):
        x=self.v_proj(x)  # (batch, channel, height, length)
        y=self.q_proj(y.unsqueeze(2).unsqueeze(2))  # (batch, channel, 1, 1)
        return self.norm(x*y)  # (batch, channel, height, length)
