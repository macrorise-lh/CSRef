...
class EMA(object):
    '''
        apply exponential moving average to a model. This should have same function as the `tf.train.ExponentialMovingAverage` of tensorflow.
        usage:
            model = resnet()
            model.train()
    ...
    def update_params(self):
        decay = min(self.alpha, (self.step + 1) / (self.step + 10))  # 1/10, 2/11, 3/12 ... --> alpha(0.9997)
        state = self.model.state_dict()
        for name in self.param_keys:
            self.shadow[name].copy_(
                decay * self.shadow[name]
                + (1 - decay) * state[name]
            )  # focus on preserving the current updated weights first, and then the previously updated weights
